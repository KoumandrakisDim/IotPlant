...
best_gb_reg = GradientBoostingRegressor(n_estimators=200, learning_rate=0.05, max_depth=5)
best_gb_reg.fit(X_train_scaled, y_train)

# Evaluate the optimized model
y_pred = best_gb_reg.predict(X_test_scaled)
mse = mean_squared_error(y_test, y_pred)
print("Mean Squared Error (Optimized Model):", mse)

# Save the model
joblib.dump(best_gb_reg, 'gradient_boosting_model.pkl')
...
predicted_evapotranspiration = best_gb_reg.predict(example_weather_scaled)
print("Predicted Evapotranspiration (Optimized Model):", predicted_evapotranspiration[0])
